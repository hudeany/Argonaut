...
public class HelpDialog extends ModalDialog<Boolean> {
	...
	public HelpDialog(final ArgonautDialog applicationDialog, final String title, final ConfigurationProperties applicationConfiguration) {
	...
	protected void createComponents(final Shell shell) throws Exception {
		...
		versionInfoButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				new ShowDataDialog(getParent(), Argonaut.APPLICATION_NAME + "(" + Argonaut.VERSION + ") " + LangResources.get("versionInfo"), VersionInfo.getVersionInfoText()).open();
			}
		});
		...
		manualButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent event) {
				String manualText;
				try (InputStream resourceStream = Argonaut.class.getResourceAsStream("/manual_" + Locale.getDefault().getLanguage().toLowerCase() + ".txt")) {
					if (resourceStream == null) {
						try (InputStream resourceStreamDefault = Argonaut.class.getResourceAsStream("/manual.txt")) {
				...
				final ShowDataDialog showDataDialog = new ShowDataDialog(getParent(), Argonaut.APPLICATION_NAME + "(" + Argonaut.VERSION + ") " + LangResources.get("manual"), manualText);
		...
		if (Utilities.isBlank(Argonaut.VERSIONINFO_DOWNLOAD_URL)) {
